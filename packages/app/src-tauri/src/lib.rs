    ...
    window.location.assign = function(url) {
        try {
            ...
            if (s.indexOf('solana-wallet:') === 0 || s.indexOf('intent:') === 0) {
                console.log('[Gaia] MWA: forwarding intent to system:', s);
...
(async () => {
    if (navigator.gpu) {
        ...
        console.log('[Gaia] WebGPU available:', !!adapter);
        if (adapter) {
            const info = await adapter.requestAdapterInfo();
            console.log('[Gaia] GPU:', info.vendor, info.architecture, info.description);
        }
    } else {
        console.warn('[Gaia] WebGPU not available in this webview');
    }
})();
"#;
...
fn setup(app: &AppHandle) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(window) = app.get_webview_window("main") {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            log::info!("Gaia: MWA intent bridge injected for Android");
        }
