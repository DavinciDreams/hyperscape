...
let _listenSocket: unknown = null;

/** Module-level uWS app reference for pub/sub broadcasting */
let _uwsApp: ReturnType<UwsModule["App"]> | null = null;
let _uwsModulePromise: Promise<UwsModule> | null = null;

async function loadUwsModule(): Promise<UwsModule> {
  if (!_uwsModulePromise) {
    _uwsModulePromise = import("uWebSockets.js");
  }
  return await _uwsModulePromise;
}
 ...
 */
export function getUwsApp(): ReturnType<UwsModule["App"]> | null {
  return _uwsApp;
}
...
export function createUwsServer(
  ...
  return loadUwsModule().then((uWS) => {
    ...
    return new Promise((resolve) => {
      app.listen(port, (listenSocket: unknown) => {
        if (listenSocket) {
...
export function closeUwsServer(): void {
  const listenSocket = _listenSocket;
  _listenSocket = null;
  _uwsApp = null;
  if (!listenSocket) {
    return;
  ...
  void loadUwsModule()
    .then((uWS) => {
      uWS.us_listen_socket_close(listenSocket as never);
      console.log("[uWS] Listen socket closed");
    })
    .catch((error: unknown) => {
      const message = error instanceof Error ? error.message : String(error);
      console.warn(`[uWS] Failed to close listen socket: ${message}`);
    });
}
