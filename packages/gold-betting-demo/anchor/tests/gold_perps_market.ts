...
describe("gold_perps_market", () => {
  ...
  it("opens and expands a long position while updating open interest", async () => {
    ...
    await program.methods
      .modifyPosition(marketId, toBn(SOL(1)), toBn(SOL(2)), toBn(0))
      .accountsPartial({
    ...
    await program.methods
      .modifyPosition(marketId, toBn(SOL(0.5)), toBn(SOL(1)), toBn(0))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        position,
  ...
  it("settles a profitable long and closes the position account", async () => {
    ...
    await program.methods
      .modifyPosition(marketId, toBn(0), toBn(-SOL(2)), toBn(0))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        position,
  ...
  it("drifts funding positive when longs dominate open interest", async () => {
    ...
    await program.methods
      .modifyPosition(marketId, toBn(SOL(1)), toBn(SOL(3)), toBn(0))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        position,
  ...
  it("liquidates underwater positions and rewards the liquidator", async () => {
    ...
    await program.methods
      .modifyPosition(marketId, toBn(SOL(1)), toBn(SOL(4)), toBn(0))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        position,
  ...
  it("keeps market state isolated across market ids", async () => {
    ...
    await program.methods
      .modifyPosition(longMarketId, toBn(SOL(1)), toBn(SOL(2)), toBn(0))
      .accountsPartial({
        config: configPda(program.programId),
        market: longMarket,
    ...
    await program.methods
      .modifyPosition(shortMarketId, toBn(SOL(1)), toBn(-SOL(3)), toBn(0))
      .accountsPartial({
        config: configPda(program.programId),
        market: shortMarket,
        position: positionPda(
  ...
  it("moves deprecated markets to close-only and allows only reductions", async () => {
    ...
    await program.methods
      .setMarketStatus(marketId, PERPS_STATUS_CLOSE_ONLY, toBn(PRICE(95)))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        authority: authority.publicKey,
    ...
    try {
      await program.methods
        .modifyPosition(marketId, toBn(SOL(1)), toBn(SOL(1)), toBn(0))
        .accountsPartial({
          config: configPda(program.programId),
          market,
          position: positionPda(
  ...
  it("archives a deprecated market once all positions are closed", async () => {
    ...
    await program.methods
      .setMarketStatus(marketId, PERPS_STATUS_CLOSE_ONLY, toBn(PRICE(100)))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        authority: authority.publicKey,
    ...
    await program.methods
      .setMarketStatus(marketId, PERPS_STATUS_ARCHIVED, toBn(PRICE(100)))
      .accountsPartial({
        config: configPda(program.programId),
        market,
        authority: authority.publicKey,
  ...
  it("rejects trades that exceed the caller's acceptable slippage", async () => {
    ...
    try {
      await program.methods
        .modifyPosition(marketId, toBn(SOL(1)), toBn(SOL(2)), toBn(PRICE(99)))
        .accountsPartial({
          config: configPda(program.programId),
          market,
