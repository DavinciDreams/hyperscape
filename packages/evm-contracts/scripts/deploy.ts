...
function resolveBettingManifestPath(): string {
  return path.resolve(
    __dirname,
    "..",
    "..",
...
function updateBettingManifest(
  ...
  const manifestPath = resolveBettingManifestPath();
...
async function main() {
  ...
  updateBettingManifest(network.name, clobAddress, goldTokenAddress);
}
