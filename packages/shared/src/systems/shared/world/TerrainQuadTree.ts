...
export interface QuadTreeConfig {
  /** Smallest chunk half-size in meters (leaf nodes) */
  minSize: number;
  /** Maximum depth of quad-tree subdivision */
  maxDepth: number;
  /** Split when distance < size * splitRatio */
  splitRatio: number;
  /** Vertex resolution at each depth (index = depth, value = segments per axis) */
  resolutionByDepth: number[];
...
export const DEFAULT_QUAD_TREE_CONFIG: QuadTreeConfig = {
  ...
  maxDepth: 4,
  splitRatio: 1.5,
  resolutionByDepth: [16, 24, 32, 48, 64],
  skirtDrop: 15,
};
...
export class TerrainQuadNode {
  ...
  /** Resolution (segments per axis) for this node's depth */
  get resolution(): number {
    const config = this.tree.config;
    return config.resolutionByDepth[
      Math.min(this.depth, config.resolutionByDepth.length - 1)
    ];
  }
...
export class TerrainQuadTree {
  ...
  update(playerX: number, playerZ: number): boolean {
    ...
    if (chunkKey === this.lastChunkKey) {
      // Still in same fine-grained cell — skip split/unsplit checks but
      // re-resolve neighbours (nodes may have been created/destroyed by
      // the visual manager's async generation pipeline since last frame).
