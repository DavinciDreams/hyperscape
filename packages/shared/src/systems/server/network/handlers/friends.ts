 ...
 *
 * Database access:
 * - Uses `world.getSystem("database") as IDatabaseSystem` for FriendRepository
...
/**
 * Get FriendRepository via DatabaseSystem
 *
 * Replaces the legacy per-world WeakMap cache — DatabaseSystem now owns the
 * single long-lived FriendRepository instance. Returns null if the database
 * system is unavailable (e.g. during PIE before DB init).
 */
function getFriendRepository(world: World): IFriendRepository | null {
  const dbSystem = world.getSystem("database") as IDatabaseSystem | undefined;
  const repo = dbSystem?.getFriendRepository?.();
  if (!repo) {
    logger.debug("Database system not available, FriendRepository unreachable");
    return null;
  }
  return repo;
}
