...
describe("PIE roundtrip — manifest drives pawn", () => {
  ...
  it("click-to-walk manifest: tick is a facade no-op (InteractionRouter owns routing)", () => {
    ...
    // Even if we dispatch input, the click-to-walk controller's tick is
    // deliberately empty — the real InteractionRouter (not present in
    // this harness) is what routes clicks. Verifying "no movement" here
    // guards against future refactors accidentally adding behavior to
    // the facade.
    pc.tick(0.1);
    pc.tick(0.5);
    pc.tick(1.0);
