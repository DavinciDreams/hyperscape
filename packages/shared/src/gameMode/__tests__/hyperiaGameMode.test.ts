/**
 * HyperiaGameMode integration tests — Phase 2.
 *
 * Validates the opt-in registration path, the composition
 * (ClickToWalk + Orbit + hyperia-default), and the lifecycle
...
import { HYPERIA_DEFAULT_CONTEXT_ID } from "../input/defaultContexts";
...
describe("HyperiaGameMode", () => {
  ...
  it("default manifest points at every canonical id", () => {
    expect(HYPERIA_DEFAULT_MANIFEST.playerController).toBe(
      CLICK_TO_WALK_CONTROLLER_ID,
    );
    expect(HYPERIA_DEFAULT_MANIFEST.camera).toBe(ORBIT_CAMERA_CONTROLLER_ID);
    expect(HYPERIA_DEFAULT_MANIFEST.inputContext).toBe(
      HYPERIA_DEFAULT_CONTEXT_ID,
    );
    expect(HYPERIA_DEFAULT_MANIFEST.pawn).toBe("humanoid-rpg");
  });
  ...
  it("resolves to a mode that composes all three controllers", () => {
    ...
    expect(mode.manifest).toBe(HYPERIA_DEFAULT_MANIFEST);
    ...
    expect(input.id).toBe(HYPERIA_DEFAULT_CONTEXT_ID);
  });
  ...
  it("activate/deactivate are no-op and idempotent", () => {
    ...
    // already owns Hyperia's native bindings.
