...
export const createManifestRoutes = (manifestService: ManifestService) => {
  return (
    new Elysia({ prefix: "/api/manifests", name: "manifests" })
      ...
      // Get manifest content (top-level manifests like npcs, stations, etc.)
      .get(
        "/:name",
      ...
      // Nested manifest routes (items/*, gathering/*, recipes/*)
      // Static prefix avoids Elysia param name conflict with /:name
      .get(
        "/items/:sub",
        async ({ params: { sub }, set }) => {
          const name = `items/${sub}`;
          const info = manifestService.getManifestInfo(name);
          if (!info) {
            set.status = 404;
            return { error: `Unknown manifest: ${name}` };
          ...
          try {
            return await manifestService.readManifest(name);
          } catch (err) {
            set.status = 404;
            return {
              error:
        ...
        {
          params: t.Object({ sub: t.String({ minLength: 1 }) }),
          response: { 200: Models.ManifestContent, 404: Models.ErrorResponse },
          detail: { tags: ["Manifests"], summary: "Get items manifest" },
        },
      )
      .get(
        "/gathering/:sub",
        async ({ params: { sub }, set }) => {
          const name = `gathering/${sub}`;
          const info = manifestService.getManifestInfo(name);
          if (!info) {
            set.status = 404;
            return { error: `Unknown manifest: ${name}` };
        ...
        },
        {
          params: t.Object({ sub: t.String({ minLength: 1 }) }),
          response: { 200: Models.ManifestContent, 404: Models.ErrorResponse },
          detail: { tags: ["Manifests"], summary: "Get gathering manifest" },
        },
      )
      .get(
        "/recipes/:sub",
        async ({ params: { sub }, set }) => {
          const name = `recipes/${sub}`;
          const info = manifestService.getManifestInfo(name);
          if (!info) {
            set.status = 404;
            return { error: `Unknown manifest: ${name}` };
        ...
        },
        {
          params: t.Object({ sub: t.String({ minLength: 1 }) }),
          response: { 200: Models.ManifestContent, 404: Models.ErrorResponse },
          detail: { tags: ["Manifests"], summary: "Get recipes manifest" },
        },
      )

      // List backups for a manifest (read-only)
      .get(
        "/:name/backups",
        async ({ params: { name }, set }) => {
          const info = manifestService.getManifestInfo(name);
