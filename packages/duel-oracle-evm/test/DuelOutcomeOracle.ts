...
type DuelOutcomeOracleContract = Awaited<
  ReturnType<Awaited<ReturnType<typeof ethers.getContractFactory>>["deploy"]>
> & {
  waitForDeployment(): Promise<unknown>;
  getDuel(duelKey: string): Promise<{
    duelKey: string;
    status: bigint;
    winner: bigint;
    seed: bigint;
...
describe("DuelOutcomeOracle", () => {
  async function deployFixture() {
    ...
    const oracle = (await DuelOutcomeOracle.connect(admin).deploy(
      admin.address,
      reporter.address,
